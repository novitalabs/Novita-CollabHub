# CloudShop with Managed Database

A fully-featured online shopping platform built with Hono, Node.js, and Artifact Hosting managed database support.

## Project Overview

- **Name**: CloudShop with Managed Database
- **Goal**: Demonstrate deploying a stateful ecommerce app with Artifact Hosting managed database support
- **Tech Stack**: Hono, Node.js, MySQL-compatible database, mysql2, TypeScript

## Database

This example is designed for Artifact Hosting's managed database integration. Deploy it through `../app.py` with `--database managed`; the platform provisions a database, runs the SQL migrations in `migrations/`, and injects `DATABASE_URL` at runtime.
